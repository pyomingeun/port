...
class HotelInfo extends Model
{
    ...
    protected $fillable = [
        'slug', 'hotel_id', 'hotel_name', 'logo', 'featured_img', 'description', 'latitude', 'longitude', 'formatted_address', 'sigun', 'sigungu', 'check_in', 'check_out', 'hotel_policy', 'terms_and_conditions','b4_0day_day_refund', 'b4_1day_refund', 'b4_2day_refund', 'b4_3day_refund', 'b4_4day_refund', 'b4_5day_refund', 'b4_6day_refund', 'b4_7day_refund', 'b4_8day_refund', 'b4_9day_refund', 'b4_10day_refund', 'rating', 'bank_detail_status','basic_info_status','address_status','hpolicies_status','fnf_status','other_info_status','summary_status','completed_percentage', 'created_by', 'updated_by', 'created_at', 'updated_at','editors_pick','min_advance_reservation','max_advance_reservation','total_payble_payout' 	 
    ];
    ...
    public function hasAttractions() {   return $this->hasMany(NearestTouristAttractions::class, "hotel_id",  "hotel_id")->orderBy('nearest_tourist_attractions.id', 'desc'); }
    ...
    public function hasFeatures()    {   return $this->hasMany(HotelFeatures::class, "hotel_id",  "hotel_id")->orderBy('hotel_features.id', 'desc'); }
    
    public function hasFacilities()    {   return $this->hasMany(HotelFacilities::class, "hotel_id",  "hotel_id")->orderBy('hotel_facilities.id', 'desc'); }
